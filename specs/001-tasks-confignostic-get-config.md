...
- [x] 6 Provide caching layer and `clear_cache()` API with thread-safe data handling.
  - [x] 6.1 Define cache keys that include canonical path, section, validator identity, and override state.
  - [x] 6.2 Wrap load execution with caching, returning copies where needed to avoid shared mutable state.
  - [x] 6.3 Expose `clear_cache()` and ensure validator types that cannot be hashed bypass caching gracefully.
- [x] 7 Compose `get_config` loader workflow with diagnostics, logging, and error shaping.
  - [x] 7.1 Orchestrate discovery, overrides, validation, and caching to return a single validated candidate.
  - [x] 7.2 Raise `ConfigSourceNotFoundError` or `ConfigSourceAmbiguityError` with aggregated diagnostics when invariants fail.
  - [x] 7.3 Emit debug-level logging for discovery attempts, parser selection, and validator outcomes.
